...
def representative_data_gen():
    ...
    for m in range(len(SNR_dB)):
        ...
        if m == 0:
            train_data,_ = torch.utils.data.random_split(train_ds, [int((1/7)*len(train_ds)), len(train_ds) - int((1/7)*len(train_ds))]) 
        else:
            temp_t,_ = torch.utils.data.random_split(train_ds, [int((1/7)*len(train_ds)), len(train_ds) - int((1/7)*len(train_ds))])
            train_data = ConcatDataset([train_data,temp_t])
        train_loader = DataLoader(train_data, batch_size=1, shuffle=True,num_workers=8,pin_memory=True,persistent_workers=True,prefetch_factor=2,worker_init_fn=worker_init_fn)
...
def main(args):
    ...
    Pd_modelIQ = np.zeros(len(SNR_dB))
    Pd_modelSCM = np.zeros(len(SNR_dB))
    test_data = {}
    for m in range(len(SNR_dB)):
        ...
        if m == 0:
            train_data,_ = torch.utils.data.random_split(train_ds, [int((1/args.SNRs)*len(train_ds)), len(train_ds) - int((1/args.SNRs)*len(train_ds))]) 
        else:
            temp_t,_ = torch.utils.data.random_split(train_ds, [int((1/args.SNRs)*len(train_ds)), len(train_ds) - int((1/args.SNRs)*len(train_ds))])
            train_data = ConcatDataset([train_data,temp_t])

        # for i in range(10):
        #     try:
        #         x, y = train_data[i]
        #         print(f"样本 {i}: x.shape={x.shape}, y.shape={y.shape if hasattr(y, 'shape') else 'scalar'}, y={y}")
        #     except Exception as e:
        #         print(f"样本 {i} 出错: {e}")
        # train_loader = DataLoader(train_data, batch_size=2,num_workers=0,pin_memory=True)
        # for x, y in train_loader:
        #     print(x.shape)
        # num_samples = 1000
        # indices = np.random.choice(len(test_ds), num_samples, replace=False)
        # test_ds = Subset(test_ds, indices)
        test_loader = DataLoader(test_ds, batch_size=1,shuffle=True,num_workers=0,pin_memory=True,worker_init_fn=worker_init_fn)
    ...
    if SCM:
        ...
        early_stopper2 = EarlyStoppingWithLR(
            ...
            mode='max',
            path=prefix_s+f'weight/{args.model2}'+'6_0_5.pt'
        )
    ...
    stop_patience=10,     # 连续 10 次不提升 → 停止训练
    mode='max',
    path=prefix_s+f'weight/{args.model1}'+'6_0_5.pt'
)
    

    train_loader = DataLoader(train_data, batch_size=args.batch_size, shuffle=True,num_workers=8,pin_memory=True,persistent_workers=True,prefetch_factor=2,worker_init_fn=worker_init_fn)
    ...
    if SCM:
        model2 = model_registry.get_model_class(args.model2)
        model2 = model2().to(DEVICE)
        model2.load_state_dict(torch.load(prefix_s+f'weight/{args.model2}'+'6_0_5.pt'))
        model2.eval()
    ### 模型评估 ###
    model1.load_state_dict(torch.load(prefix_s+f'weight/{args.model1}'+'6_0_5.pt'))
    model1.eval()
    ...
    #             total += y.size(0)
    #     Pd_modelIQ[m] = 1- (correct1 / total)
    #     if SCM:
    #         Pd_modelSCM[m] = 1- (correct2 / total)
...
#  0.04291212]
    ...
    dummy_input = torch.randn(1, 2, 33, 150).to(DEVICE)   # 按你的输入尺寸改
    # dummy_input = torch.randn(33, 2, 150).to(DEVICE)   # 按你的输入尺寸改
    # dummy_input = torch.randn(1, 33, 150, 2).to(DEVICE)   # 按你的输入尺寸改
    ...
    # 1. 加载 SavedModel（或 .h5 模型也支持）
    # onnx_m = onnx.load('final.onnx')

    onnx_m = onnx.load('ztest1.onnx')
    tf_m = prepare(onnx_m)
    tf_m.export_graph('ztest1')

    print('开始量化')
    converter = tf.lite.TFLiteConverter.from_saved_model("ztest1")
    # converter = tf.lite.TFLiteConverter.from_saved_model("final")
    ...
    # 4) 指定只使用 int8 的算子（可完全免浮点）
    converter.target_spec.supported_ops = [
        tf.lite.OpsSet.TFLITE_BUILTINS_INT8,
    ]
    # 5) 设置输入输出张量的数据类型为 int8 和float32
    ...
    # 7) 写入文件
    with open("ztest.tflite", "wb") as f:
        f.write(tflite_quant_model)

    print("量化完成，已保存为 ztest.tflite")

    print('测试lite模型')

    interpreter = tf.lite.Interpreter(model_path="ztest.tflite")
    # interpreter = tf.lite.Interpreter(model_path="mb2_quant2.tflite")
    ...
    for m in range(len(SNR_dB)):
        ...
        total=0
        # with torch.no_grad():
        for x_pt, target in test_data[f'{m}']:
            # PyTorch tensor → NumPy float32
            x_np = x_pt.numpy().astype(np.float32)
            B, C, H, W = x_np.shape

            # 如果模型采 NHWC，转置；否则按实际签名来
            # x_np = x_np.transpose(0,2,3,1)

            # 4) 量化输入 (float32 → int8)
            x_int8 = (x_np / in_scale + in_zero).round().astype(np.int8)
        #     x_int8 = np.clip(
        #     np.round(x_np / in_scale + in_zero),
        #     -128,
        #     127
        # ).astype(np.int8)
            in_index = input_detail["index"]
            interpreter.resize_tensor_input(in_index, x_int8.shape)
            interpreter.allocate_tensors()
            ...
            interpreter.invoke()
            y_out  = interpreter.get_tensor(output_detail["index"])

            if output_detail["dtype"] == np.float32:
                y_float = y_out.astype(np.float32)
            else:
                out_scale, out_zero = output_detail["quantization"]
                y_float = (y_out.astype(np.float32) - out_zero) * out_scale
            ...
            # # 7) 转回 PyTorch，计算 Top-1/Top-5
            logits = torch.from_numpy(y_float).to(DEVICE)

            # logits = torch.from_numpy(y_int8)
            target = target.to(DEVICE)

            preds1 = torch.argmax(logits, dim=1)
            correct1 += (preds1 == target.argmax(dim=1)).sum().item()
            total += target.size(0)
            ...
            # top1_total += acc1[0].item() * B /100
        Pd_modelIQ[m] = 1- (correct1 / total)
        # Pd_modelIQ[m] = 1- (top1_total/n_samples*100)

    print(Pd_modelIQ)

    # # 8) 输出整体精度  80.92
    # print(f"Quantized model on full test set: "
    #     f"Top-1 = {top1_total/n_samples*100:.2f}%, "
    #     f"Top-5 = {top5_total/n_samples*100:.2f}%")
